...
The subscription fees are available also by [agreement
type](cost_by_type.csv), organization type, and [subscription
category](cost_by_category.csv).
...
Let us compare the [relative increase in publisher costs](table/cost_by_publisher_year_relative.csv). The costs are normalized to 1 in 2010, and the top 10 publishers with the highest cost increase in 2010-2015 are shown. The `r sum(!(!is.na(subset(dfs, Year == 2010)$Costs) & !is.na(subset(dfs, Year == 2015)$Costs)))` publishers that did not have declared costs in 2010 or 2015 (see [here](table/cost_by_publisher_year.csv)) are excluded.
...
Finally, let us compare the [relative increase in costs across the institutions](table/cost_by_organization_year_relative.csv). The costs are normalized to 1 in 2010, and the top 10 organizations with the highest cost increase in 2010-2015 are shown. The `r sum(!(!is.na(subset(dfs, Year == 2010)$Costs) & !is.na(subset(dfs, Year == 2015)$Costs)))` organizations that did not have declared costs in 2010 or 2015 (see [here](table/cost_by_organization_year.csv)) are excluded.
